...
def transaction_kind(description):
    text = str(description or "").lower()
    if "injured list" in text or "disabled list" in text:
        return "injured", "Injured List"
    if "rehab assignment" in text:
        return "rehab", "Rehab Assignment"
    if "recalled" in text or "selected the contract" in text or "contract selected" in text:
        return "callup", "Call-Up"
    if "activated" in text:
        return "activated", "Activated"
    if "optioned" in text:
        return "optioned", "Optioned"
    if "assigned" in text:
    ...
    return "move", "Roster Move"


def build_transactions():
    rows=[]
    since=(datetime.now(PACIFIC).date()-timedelta(days=14)).isoformat()
    until=datetime.now(PACIFIC).date().isoformat()
    for p in PLAYERS:
        try:
            data=get_json(
                f"https://statsapi.mlb.com/api/v1/people/{p['id']}",
                {"hydrate":f"transactions(startDate={since},endDate={until})"}
            )
            person=(data.get("people") or [{}])[0]
            for t in person.get("transactions",[])[:5]:
                desc=t.get("description") or t.get("typeDesc") or "Roster transaction"
                kind,label=transaction_kind(desc)
                rows.append({
                    "player":p["name"],
                    "playerId":p["id"],
                    "date":t.get("date"),
...
def main():
    ...
    if args.mode == "full":
        ...
        print(f"Wrote live refresh: {OUTPUT}, {SCHEDULE_OUTPUT}, {TRANSACTIONS_OUTPUT}")
