...
function playerCard(p){return`<article class="player-card"><div class="player-top">${portrait(p)}<div><div class="player-name">${esc(p.name)}</div><div class="assignment"><b>${esc(p.position)}</b><br>${esc(p.recentTeam||p.team)} · ${esc(normalizedLevel(p))}</div><span class="level-badge">${esc(displayStatusLabel(p))}</span></div>${p.teamId?`<img class="team-logo" src="${logo(p.teamId)}" alt="">`:''}</div><div class="stats ${p.type==='pitcher'?'pitcher':''}">${orders(p).map(k=>`<div class="stat"><b>${esc(p.stats?.[k]??'—')}</b><small>${k}</small></div>`).join('')}</div><div class="player-note">${esc(p.note||'')}</div><div class="card-actions"><a href="player.html?id=${p.mlbId}">View Mustang profile</a>${p.profileUrl?`<a href="${esc(p.profileUrl)}" target="_blank" rel="noopener">Official profile ↗</a>`:''}</div></article>`}
